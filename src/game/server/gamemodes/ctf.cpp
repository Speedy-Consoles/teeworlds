...
bool CGameControllerCTF::OnEntity(int Index, vec2 Pos, int SwitchGroup, bool InvertSwitch)
{
	...
	CFlag *F = new CFlag(GameServer()->m_TeamCore.GetTeamWorld(0), Team);
	F->m_StandPos = Pos;
	F->m_Pos = Pos;
	m_apFlags[Team] = F;
	GameServer()->m_TeamCore.GetTeamWorld(0)->InsertEntity(F);
	return true;
}
...
void CGameControllerCTF::Tick()
{
	...
	if(GameServer()->m_TeamCore.GetTeamWorld(0)->m_ResetRequested || GameServer()->m_TeamCore.GetTeamWorld(0)->m_Paused)
		return;
	...
	for(int fi = 0; fi < 2; fi++)
	{
		...
		else
		{
			...
			int Num = GameServer()->m_TeamCore.GetTeamWorld(0)->FindEntities(F->m_Pos, CFlag::ms_PhysSize, (CEntity**)apCloseCCharacters, MAX_CLIENTS, CGameWorld::ENTTYPE_CHARACTER);
			for(int i = 0; i < Num; i++)
			{
			...
			if(!F->m_pCarryingCharacter && !F->m_AtStand)
			{
				if(Server()->Tick() > F->m_DropTick + Server()->TickSpeed()*30)
				{
					...
					F->m_Vel.y += GameServer()->m_TeamCore.GetTeamWorld(0)->m_Core.m_Tuning.m_Gravity;
